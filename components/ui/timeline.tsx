      ...
      <div ref={ref} className="relative max-w-4xl mx-auto pb-20">
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:pt-20 md:gap-10">
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:pl-20 md:text-3xl font-semibold text-neutral-500">
                {item.title}
              </h3>
            </div>
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-semibold text-neutral-500">
                {item.title}
              </h3>
              {item.content}
            </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0 w-full bg-gradient-to-t from-signal via-signal to-transparent rounded-full"
          />
        </div>
      </div>
    </div>
